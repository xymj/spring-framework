...
public abstract class AbstractAutoProxyCreator extends ProxyProcessorSupport
	...
	public Object postProcessBeforeInstantiation(Class<?> beanClass, String beanName) throws BeansException {
		...
		// 正常来说Aop的代理创建应当在Bean创建后再进行代理类，但是这里在Bean创建前就可能进行了代理：
		...
		if (targetSource != null) {
			...
			// 简单来说就是两步
			//	AnnotationAwareAspectJAutoProxyCreator#findCandidateAdvisors挑选出所有的Advisor。
			//		在其中通过 super.findCandidateAdvisors()调用了AbstractAdvisorAutoProxyCreator#findCandidateAdvisors来完成了对硬编码注入的Advisor的获取解析返回。
			//		随后通过this.aspectJAdvisorsBuilder.buildAspectJAdvisors()方式解析了 Aop 注解方式动态封装的Advisor并保存。
	 ...
	 */
	// 判断当前bean是否需要代理，如果需要，则进行bean封装
	protected Object wrapIfNecessary(Object bean, String beanName, Object cacheKey) {
		// 如果当前bean已经处理过了直接返回
		if (StringUtils.hasLength(beanName) && this.targetSourcedBeans.contains(beanName)) {
			return bean;
		}
		// 无需增强。这个在AbstractAutoProxyCreator#postProcessBeforeInstantiation方法中对cacheKey进行了判断缓存，
	...
	// targetSource : SingletonTargetSource 目标类是单例 拥有给定对象的TargetSource接口的实现。
	...
	protected Object createProxy(Class<?> beanClass, @Nullable String beanName,
		...
		if (!proxyFactory.isProxyTargetClass()) {
			// 检查 proxyTargeClass设置以及preserveTargetClass属性
	...
	protected Advisor[] buildAdvisors(@Nullable String beanName, @Nullable Object[] specificInterceptors) {
		// specificInterceptors应该全是InstantiationModelAwarePointcutAdvisorImpl类型
